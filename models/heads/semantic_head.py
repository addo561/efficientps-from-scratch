...
def Separableconvolution(ch_in,ch_out,expansion,custom_stride,k, dilate:tuple=None,padding:tuple=None):
    ...
    block = nn.Sequential(
            nn.Conv2d(
                ...
                padding= padding if padding else k//2,
                bias=False,
                dilation= dilate if dilate else None
                ),
            nn.Conv2d(
            in_channels=ch_in,
            out_channels=ch_out,
...
class LSFE(nn.module):
    ...
    def __init__(self,in_ch=256,out_ch = 128):
        ...
        self.SeparableConv = Separableconvolution(ch_in=in_ch,
                                                  ch_out=out_ch,
                                                  expansion=1,
                                                  k=3,
                                                  custom_stride=1)
...
class DPC(nn.Module):
    ...
    def __init__(self,channels=256):
        ...
        self.conv = nn.Conv2d(in_channels=256,out_channels=128,kernel_size=1,stride=1)
    def forward(self,x):
